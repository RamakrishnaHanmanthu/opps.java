...
public class AllStreams {
	...
	public static void main(String[] args) {
		...
		boolean testing = testnames.stream().allMatch(n -> n.startsWith("t"));
		System.out.println("the first " + testing);

		boolean testingAny = testnames.stream().anyMatch(n -> n.contains("14"));
		...
		Optional<String> findFirst = testnames.stream().findFirst();
		...
		List<String> nameOfSec = Arrays.asList("The words", "let's how", "it works");

		// Flatten the stream of sentences into a stream of words
		List<String> flatmaps = nameOfSec.stream().flatMap(n -> Arrays.stream(n.split(" ")))
				.collect(Collectors.toList());
		System.out.println(flatmaps);

		List<List<String>> nameOfSecs = Arrays.asList(Arrays.asList("one", "two"), Arrays.asList("three", "four"),
				Arrays.asList("five", "six"));

		// Flatten the list of lists into a single list
		List<String> flatmap = nameOfSecs.stream().flatMap(n -> n.stream())// (List::stream)
				.collect(Collectors.toList());
		...
		String reduceString = nameOfSec.stream().reduce("", (n1, n2) -> n1 + " " + n2); // (String::concat)
		System.out.println(reduceString);

		List<Integer> mapNameLeng = testnames.stream().map(n -> n.length()).collect(Collectors.toList());
		System.out.println(mapNameLeng);

		Optional<Integer> maxNum = intStream.stream().max(Comparator.naturalOrder());
		maxNum.ifPresent(n -> System.out.println(n));

		Optional<Integer> minNum = intStream.stream().min(Comparator.naturalOrder());
		...
		List<String> mapSkip = testnames.stream().skip(2).collect(Collectors.toList());
		System.out.println(mapSkip);

		List<Integer> intSort = Arrays.asList(10, 29, 89, 9);

		List<Integer> sorted = intSort.stream().sorted().collect(Collectors.toList());
		System.out.println(sorted);

		List<String> peek = testnames.stream().filter(n -> n.length() > 2).peek(System.out::println)
				.map(String::toUpperCase).peek(m -> System.out.println(m)).collect(Collectors.toList());
